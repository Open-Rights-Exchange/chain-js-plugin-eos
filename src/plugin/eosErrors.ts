...
export const ChainErrorRegExs: { [key: string]: string } = {
  ...
  AccountDoesntExist: 'account_query_exception|account does not exist', // the account not on chain. Thrown by functions like link permission. Not thrown by get_account which throws 'unknown key \\(boost'
...
export function mapChainError(error: RpcError | Error): ChainError {
  ...
  if (error instanceof RpcError) {
    errorSearchString = `${error.name} ${error.message} ${Helpers.stringifySafe(error.json)}` // includes the full body of the response from the HTTP request to the chain
    errorMessage = `${Helpers.stringifySafe(error.json)}`
