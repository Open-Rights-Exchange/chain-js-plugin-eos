...
export class PermissionsHelper {
  ...
  async composeAddPermissionActions(
    ...
    if (Helpers.isNullOrEmpty(permissionsToAdd)) return null
  ...
  composeDeletePermissionActions = async (
    ...
    if (Helpers.isNullOrEmpty(permissionsToDelete)) return null
    ...
    permissionsToDelete.map(async auth => {
      ...
      const deleteAuthAction = await composeAction(Models.ChainActionType.AccountDeleteAuth, deleteAuthParams)
  ...
  composeReplacePermissionKeysAction = async (
    ...
    if (Helpers.isNullOrEmpty(accountPermissions)) return null
  ...
  composeLinkPermissionActions = async (
    ...
    if (Helpers.isNullOrEmpty(permissionsToLink)) return null
    ...
    permissionsToLink.map(async link => {
      ...
      const linkAuthAction = await composeAction(Models.ChainActionType.AccountLinkAuth, linkAuthParams)
  ...
  composeUnlinkPermissionActions = async (
    ...
    if (Helpers.isNullOrEmpty(permissionsToUnlink)) return null
    ...
    permissionsToUnlink.map(async link => {
      ...
      const unlinkAuthAction = await composeAction(Models.ChainActionType.AccountUnlinkAuth, unlinkAuthParams)
  ...
  static generateMissingKeysForPermissionsToAdd = async (
    ...
    if (Helpers.isNullOrEmpty(permissionsToAdd)) {
      return null
    }
  ...
  static mapPermissionStructToEosPermission = (accountPermissionStruct: EosPermissionStruct): EosPermission => {
    ...
    if (Helpers.isNullOrEmpty(required_auth)) return null
