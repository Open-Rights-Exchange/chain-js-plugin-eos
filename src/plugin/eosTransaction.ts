...
export type PermissionMapCache = {
  ...
  permission: EosRequiredAuthorization
}
...
export class EosTransaction implements Interfaces.Transaction {
  ...
  public get missingSignatures(): EosAuthorizationPerm[] {
    this.assertIsValidated()
    const missing = this._requiredAuthorizations?.filter(auth => {
      auth.requiredAuthorization.keys.forEach(keyObj => {
        return !this.hasSignatureForPublicKey(keyObj.key)
      })
    })
  ...
  public get isMultisig(): boolean {
    ...
    this._requiredAuthorizations?.forEach(auth => {
      console.log(auth.requiredAuthorization.keys)
