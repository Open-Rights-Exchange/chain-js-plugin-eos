...
export class EosAccount implements Interfaces.Account {
  ...
  async load(accountName: EosEntityName): Promise<void> {
    ...
    try {
      ...
      if (chainError.errorType === Models.ChainErrorType.DataReadFailedKeyDoesNotExist) {
        chainError.errorType = Models.ChainErrorType.AccountDoesntExist
      }
