...
export class EosChainState implements Interfaces.ChainState {
  private eosChainInfo: RpcInterfaces.GetInfoResult

  private _activeEndpoint: EosChainEndpoint

  private _chainInfo: Models.ChainInfo
  ...
  private selectEndpoint(): { url: string; endpoint: EosChainEndpoint } {
    ...
    if (Helpers.isNullOrEmpty(this.endpoints)) {
  ...
  private configureFetchForEndpoint(fetch: any, chainEndpoint: EosChainEndpoint): any {
    const endpointHeaders = chainEndpoint ? Helpers.arrayToObject(chainEndpoint?.options?.headers) : null
    if (Helpers.isNullOrEmpty(endpointHeaders)) return fetch
    const headers = new NodeFetchHeaders(endpointHeaders)
    // add chainEndpoint headers to fetch headers
    return Helpers.fetchWrapper(fetch, { headers })
  }
  ...
  public async fetchBalance(
    ...
    if (!Helpers.isNullOrEmpty(balance) && Helpers.isAString(balance)) {
  ...
  async sendTransaction(
    ...
    if (
      waitForConfirm !== Models.ConfirmType.None &&
      waitForConfirm !== Models.ConfirmType.After001 &&
      waitForConfirm !== Models.ConfirmType.Final
    ) {
      Errors.throwNewError('EOS plugin only supports ConfirmType.None, .After001, and .Final')
    }
  ...
  */
  ...
  private async checkIfAwaitConditionsReached(
    ...
    try {
      ...
      if (hasReachedConfirmLevel) {
        Errors.resolveAwaitTransaction(resolve, transactionResult)
        return
      }
  ...
  public async fetchTransaction(
    ...
    if (!txHistory) throw new ChainError(Models.ChainErrorType.TxNotFoundOnChain, 'Transaction Not Found')
    ...
    if (txHistory.receipt.status === EosTransactionHistoryStatus.Executed) {
      ...
      status = Models.TransactionStatus.Dead
    }
